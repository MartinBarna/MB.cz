...
Deno.serve(async (req: Request) => {
  ...
  let sent = 0, skippedAlready = 0, errors = 0, finished = 0, stopped = 0, gaveUp = 0, bridged = 0, skippedOwns = 0, skippedZapis = 0, delayedZapis = 0, delayedKonzultace = 0, pausedNeaktivita = 0, rescuedNeaktivita = 0, capped = false, timeUp = false; const byStep: Record<string, number> = {}; const byBridge: Record<string, number> = {};
  for (const l of leads) {
    ...
    if (konzultaceVBehu(String(l.track || ''), String(l.email).toLowerCase(), konzultaceCekaNaHovor)) { const nextKonz = new Date(Date.now() + ODLOZ_MS).toISOString(); await admin.from('leads').update({ next_send_at: nextKonz, updated_at: nowIso }).eq('id', l.id); await admin.from('email_events').insert({ lead_id: l.id, step: l.step, type: 'odklad_konzultace', detail: { track: l.track, key: tpl.key, duvod: 'ceka na konzultacni hovor s Martinem' } }); delayedKonzultace++; continue; }
