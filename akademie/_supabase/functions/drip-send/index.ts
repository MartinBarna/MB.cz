...
// Ceny v sablonach (23. 9. 2026): ceny appky z `pricing_plans`, ceny Academy z `app_config`.
// ⛔ Nenactena cena NENI cena: mail s cenovou promennou pocka na dalsi beh, viz `_shared/ceny.ts`.
import { alertCeny, chybejiciCeny, nactiCeny, slevyVideokurzu } from '../_shared/ceny.ts';
...
function buildVars(name: string, seg: Seg, unsub: string, email: string, extra: Record<string, unknown> | null, cisla: Record<string, string>): Record<string, string> {
  ...
  const vestavene: Record<string, string> = {
    ...
    // Ceny (`course_price`, `discount_price`, `cena_*`) prichazeji v `cisla` z `_shared/ceny.ts`.
    // ⛔ Zadna konstanta natvrdo: kdyz se cena nenacte, v `cisla` chybi a mail pocka.
...
function renderEmail(tpl: Tpl, seg: Seg, v: Record<string, string>, footer: { html: string; text: string }) {
  ...
  // Jmeno promenne patri do chyby: bez nej se z `email_events` neda poznat, co v sablone chybi.
  if (hasToken(subject) || hasToken(html) || hasToken(text)) throw new Error('unresolved_token:' + [...new Set((subject + ' ' + text).match(/\{\{[^{}]*\}\}|\[\[|\]\]|\[a\]|\[á\]/g) ?? [])].join(',').slice(0, 160));
...
Deno.serve(async (req: Request) => {
  ...
  // CENY (23. 9. 2026). Nactou se jednou za beh; co se nenacetlo, v CISLA CHYBI (zadna zaloha).
  const ceny = await nactiCeny(admin);
  const cenyStav = ceny.chyby.length ? 'chyba: ' + ceny.chyby.join('; ').slice(0, 300) : 'ok';
  const CISLA = { pocet_potravin: (fMap.pocet_potravin ?? '').trim() || '50 000', pocet_receptu: (fMap.pocet_receptu ?? '').trim() || '140', ...ceny.hodnoty, ...slevyVideokurzu(ceny, { discount_price: DISCOUNT_PCT, discount2_price: DISCOUNT2_PCT }) };
  ...
  if (typeof body.test_email === 'string' && body.test_email.includes('@')) {
    ...
    { const chybi = chybejiciCeny(tpl, CISLA); if (chybi.length) return json({ ok: false, mode: 'test', error: 'ceny_nenacteny', chybi, ceny: cenyStav }, 503); }
  ...
  if (typeof body.oneoff_email === 'string' && body.oneoff_email.includes('@')) {
    ...
    // ⛔ [R1, N1] Oneoff volaji i SQL funkce, ktere odpoved NECTOU (`tc_kosik_zapis` razitkuje predem).
    //    503 bez alertu by byla ticha ztrata mailu, proto alert (nejvys 1 za 6 h).
    { const chybi = chybejiciCeny(tpl, CISLA); if (chybi.length) { await alertCeny(admin, RESEND_KEY, 'drip-send:ceny-oneoff', '[VYRIDIT] drip-send: jednorazovy mail s cenou NEODESEL, ceny se nenacetly', 'Jednorazovy mail (oneoff) ' + track + '/' + step + ' NEODESEL a SAM SE NEZOPAKUJE (volajici odpoved necte). Prvni zasazeny: ' + to + '. Dalsi v nasledujicich 6 h uz vlastni alert nedostanou, hledej je podle trate.\n\nChybi promenne: ' + chybi.join(', ') + '\nDuvod nacteni: ' + cenyStav + '\n\nCo s tim: az se ceny nactou, posli ho znovu rucne (drip-send oneoff_email), nebo cenu ze sablony odstran.'); return json({ ok: false, mode: 'oneoff', error: 'ceny_nenacteny', chybi, ceny: cenyStav }, 503); } }
  ...
  // PAUZA PRED KONZULTACNIM HOVOREM (15. 9. 2026). Mnozina lidi, kteri maji zaplacenou
  // konzultaci a hovor s Martinem je JESTE CEKA. Termin zadava Martin v adminu do tabulky
  // `consultation_calls`; chybejici termin i termin v budoucnu znamena "hovor jeste nebyl".
  // Vysledek se predava cistou funkci `konzultaceVBehu` (pravidla.ts), aby sla testovat bez site.
  // FAIL-OPEN: kdyz kterykoli z tech dvou dotazu selze (treba proto, ze tabulka jeste
  // neexistuje, protoze se nasadila funkce driv nez migrace), mnozina zustane PRAZDNA
  // a maily jdou dal. Chyba dotazu neni odpoved dotazu; tise zadrzet maily VSEM by bylo
  // horsi nez poslat jeden mail navic. Stav jde do odpovedi funkce, at to selhani neni tiche.
  const { data: konzRows, error: konzErr } = await admin.from('entitlements').select('email').eq('product', 'konzultace').eq('active', true).or('expires_at.is.null,expires_at.gt.' + new Date().toISOString());
  const { data: hovorRows, error: hovorErr } = await admin.from('consultation_calls').select('email,termin_at');
  const konzultaceCekaNaHovor = new Set<string>();
  let konzBrana: string;
  if (konzErr || hovorErr) { konzBrana = 'chyba_dotazu_posilame:' + String(konzErr?.message ?? hovorErr?.message ?? '').slice(0, 80); }
  else {
    const poHovoru = new Set<string>((hovorRows ?? []).filter((r: { termin_at: string | null }) => !!r.termin_at && Date.parse(String(r.termin_at)) <= Date.now()).map((r: { email: string }) => String(r.email ?? '').toLowerCase()).filter(Boolean));
  ...
  let sent = 0, skippedAlready = 0, errors = 0, finished = 0, stopped = 0, gaveUp = 0, bridged = 0, skippedOwns = 0, skippedZapis = 0, delayedZapis = 0, delayedKonzultace = 0, odznackovanoKonzultace = 0, pausedNeaktivita = 0, rescuedNeaktivita = 0, capped = false, timeUp = false; const byStep: Record<string, number> = {}; const byBridge: Record<string, number> = {};
  // Maily, ktere cekaji na ceny, a sablony s neznamou promennou. Obe veci konci alertem Martinovi.
  let odlozenoCeny = 0; const chybiCenyNazvy = new Set<string>(); const odlozenoCenyKde = new Set<string>(); const neznamePromenne = new Set<string>();
  for (const l of leads) {
    ...
    // ODKLAD PRED KONZULTACNIM HOVOREM. Schvalne TESNE ZA `shouldStop` a PRED `maPreskocitKrok`:
    // je to nejsilnejsi duvod dnes nic neposilat. ODKLAD, ne stop: `status` zustava 'active',
    // takze clovek neprijde o blog-newsletter ani o tydenik, a po hovoru trat proste pokracuje.
    ...
    // ⛔ CENA SE NENACETLA => tenhle mail dnes NEODEJDE. Ne `error` (ten by se pocital do MAX_TRIES,
    //    zaparkoval by leada a zivy jistic `followups_circuit_breaker` by pri deseti chybach zavrel
    //    follow-upy cele Academy), ale tichy odklad: `next_send_at` se nemeni, dalsi hodinovy beh
    //    to zkusi znovu. Martin dostane alert (nejvys jeden za 6 h).
    { const chybi = chybejiciCeny(tpl, CISLA); if (chybi.length) { odlozenoCeny++; for (const c of chybi) chybiCenyNazvy.add(c); odlozenoCenyKde.add(l.track + '/' + krokSend); continue; } }
    try { const varsZLeada = (l.vars && typeof l.vars === 'object' && !Array.isArray(l.vars)) ? (l.vars as Record<string, unknown>)[String(l.track)] as Record<string, unknown> | undefined : undefined; const v = buildVars(String(l.name ?? ''), seg, SUPABASE_URL + '/functions/v1/unsubscribe?token=' + l.unsubscribe_token, String(l.email), extraVars ?? varsZLeada ?? null, CISLA); const m = renderEmail(tpl, seg, v, footer); const htmlSeStopou = await ostopkuj(m.html, { track: String(l.track), step: krokSend, key: tpl.key, lead_id: String(l.id) }, MAIL_TRACK_SECRET, SUPABASE_URL, !archiveBcc); await pace(); const id = await sendViaResend(l.email, m.subject, htmlSeStopou, m.text, v.unsubscribe_url, replyTo, archiveBcc); const sentDetail = rescueThenPause ? { track: l.track, key: tpl.key, cadence: 'rescue' } : { track: l.track, key: tpl.key }; const { error: logErr } = await admin.from('email_events').insert({ lead_id: l.id, step: krokSend, type: 'sent', provider_id: id, detail: sentDetail }); if (logErr && !String(logErr.code).includes('23505')) throw new Error('log:' + logErr.message); if (rescueThenPause) { const podminkaPauzy = akce.typ === 'rescue_krok' || akce.typ === 'rescue_pauza' ? akce.podminka : 'jen_kdyz_aktivni'; await pauzniNeaktivitu(krokSend, tpl.key, podminkaPauzy); rescuedNeaktivita++; } else await advance(); sent++; const k = l.track + '/step' + krokSend + ':' + tpl.key; byStep[k] = (byStep[k] ?? 0) + 1; } catch (e) { errors++; if (String(e).includes('unresolved_token')) neznamePromenne.add(l.track + '/' + krokSend + ': ' + String(e).slice(0, 160)); await admin.from('email_events').insert({ lead_id: l.id, step: l.step, type: 'error', detail: { track: l.track, error: String(e).slice(0, 400) } }); const { count: failCount } = await admin.from('email_events').select('id', { count: 'exact', head: true }).eq('lead_id', l.id).eq('step', l.step).eq('type', 'error').eq('detail->>track', l.track); const jeClensky = CLENSKE_PREFIXY.some((p) => String(l.track || '').startsWith(p)); if (jeClensky && (failCount ?? 0) >= MAX_TRIES) { if ((failCount ?? 0) === MAX_TRIES) await admin.from('email_events').insert({ lead_id: l.id, step: l.step, type: 'gave_up_warn', detail: { track: l.track, tries: failCount ?? 0 } }); const retry = new Date(Date.now() + 6 * 3600000).toISOString(); await admin.from('leads').update({ next_send_at: retry, updated_at: nowIso }).eq('id', l.id); } else if ((failCount ?? 0) >= MAX_TRIES) { await admin.from('leads').update({ status: 'paused', next_send_at: null, updated_at: nowIso }).eq('id', l.id); await admin.from('email_events').insert({ lead_id: l.id, step: l.step, type: 'gave_up', detail: { track: l.track, tries: failCount ?? 0 } }); gaveUp++; } else { const retry = new Date(Date.now() + 6 * 3600000).toISOString(); await admin.from('leads').update({ next_send_at: retry, updated_at: nowIso }).eq('id', l.id); } }
  }
  // ⛔ Alerty AZ PO smycce: jeden souhrn za beh, ne mail za kazdeho leada.
  if (odlozenoCeny > 0) await alertCeny(admin, RESEND_KEY, 'drip-send:ceny', '[VYRIDIT] drip-send: ' + odlozenoCeny + ' mailu s cenou ceka, ceny se nenacetly',
    'Maily s cenovou promennou dnes NEODESLY a cekaji na dalsi hodinovy beh (nic se neztratilo, lead zustava ve fronte).\n\n'
    + 'Chybi promenne: ' + [...chybiCenyNazvy].join(', ') + '\nSablony: ' + [...odlozenoCenyKde].slice(0, 20).join(', ') + '\n\n'
    + 'Duvod nacteni: ' + cenyStav + '\n\n'
    + 'Co s tim: ceny appky jsou v pricing_plans (appka), ceny Academy v app_config klice cena_* (Academy). '
    + 'Kdyz je to vypadek site, samo se to srovna. Kdyz klic chybi nebo ma nesmysl, oprav ho.');
  if (neznamePromenne.size > 0) await alertCeny(admin, RESEND_KEY, 'drip-send:nezname', '[VYRIDIT] drip-send: sablona ma promennou, kterou engine nezna',
    'Tyhle maily NEODESLY (render spadl na unresolved_token, pocita se to jako chyba a zkousi se znovu po 6 h):\n\n'
    + [...neznamePromenne].slice(0, 20).join('\n') + '\n\nCo s tim: oprav preklep v sablone (email_templates), nebo promennou dopln do enginu.');
  return json({ ok: true, mode: 'live', ceny: cenyStav, odlozeno_ceny: odlozenoCeny, stopa: stopaStav, followups_enabled: followupsEnabled, due: leads.length, sent, daily_cap: DAILY_CAP, send_gap_ms: SEND_GAP_MS, max_tries: MAX_TRIES, pools: poolInfo, sent_today_before: sentToday ?? 0, remaining_today: remaining, capped, time_up: timeUp, skipped_already: skippedAlready, stopped_bought: stopped, skipped_owns: skippedOwns, skipped_zapis: skippedZapis, delayed_zapis: delayedZapis, delayed_konzultace: delayedKonzultace, odznackovano_konzultace: odznackovanoKonzultace, konzultace_brana: konzBrana, paused_neaktivita: pausedNeaktivita, rescued_neaktivita: rescuedNeaktivita, zapis_signal: aktivaceStav, finished, errors, gave_up: gaveUp, bridged, by_bridge: byBridge, mosty: Object.keys(MOSTY), zapis_kroky: Object.keys(KROK_PODLE_ZAPISU), okno_neaktivity_dni: OKNO_NEAKTIVITY_DNI, by_step: byStep });
});
