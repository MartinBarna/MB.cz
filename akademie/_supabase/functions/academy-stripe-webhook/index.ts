 ...
 * ⚠️ ČISTÁ FUNKCE, jen preference. Existenci šablony řeší `vyberRozlouceniTrack()` níž.
 */
...
// ⛔⛔ [17. 9. 2026, nález A/N9] ŠABLONA `rozlouceni-refund-dojezd` V DB NEEXISTUJE.
// Změřeno 16. i 17. 9. 2026: `select track, step from email_templates where track like
// 'rozlouceni-%'` vrací JEDINÝ řádek, `rozlouceni-refund-hned/0`.
// Dnešní jediné volání předává `konecIso` = teď, takže vždycky vyjde `hned` a nikdo to
 ...
 *
 * ⚠️ FILTR SE PŘEDÁVÁ ZVENČÍ, protože párovací klíče jsou DVA a nedají se sloučit:
 *    `order_id` u všeho, co zapisuje tenhle webhook, a e-mail plus okno u appky
 *    (viz `oknoProvizeAppkyIso`). Tělo, tedy čtení, zápis i alert, je pro obě cesty
 *    stejné a nesmí se rozejít.
 *
...
async function zneplatniProvizeRefundem(
  // Doplní na dotaz párovací podmínky. Tentýž filtr jede na čtení i na zápis.
  // deno-lint-ignore no-explicit-any
  filtr: (q: any) => any,
  /** Jak se to spárovalo. Jde do alertu, ať Martin pozná, podle čeho se rušilo. */
  popisParovani: string,
  jeSpor: boolean,
): Promise<string> {
  const slovoUdalosti = jeSpor ? "Spor" : "Refund";
  try {
    const { data: radky, error: chybaCteni } = await filtr(
    ...
    const { error: chybaZapisu } = await filtr(
      admin.from("referrals").update({ status: STAV_REFUND }),
    ).neq("status", STAV_REFUND);
    ...
    await alertAdmin(
      potvrzene.length
        ? "🔴 " + slovoUdalosti + ": zrušena UŽ POTVRZENÁ provize"
        : slovoUdalosti + ": provize zrušena, partner ji nedostane",
      {
        zruseno_radku: radky.length,
        detail: souhrn,
        sparovano_podle: popisParovani,
        // ⛔ Věta o sporu patří do OBOU větví, proto je stavba textu ve `provize.ts`
        //    a otestovaná; napsaná tady za ternárním operátorem se kvůli prioritě
        //    `+` proti `?:` lepila jen k jedné z nich.
        co_delat: coDelatPoZruseniProvize(potvrzene.length, jeSpor),
      },
    );
    ...
    await alertAdmin("🔴 Stripe: provize se po refundu NEZRUŠILA", {
      ...
      co_delat: "⛔ Přepiš `referrals.status` na `void` ručně podle párování výš. "
        + "Jinak se provize po 14 dnech sama potvrdí a partnerovi se vyplatí "
        + "odměna za nákup, za který jsme vrátili peníze." + vetaOSporu(jeSpor),
    });
    return "chyba";
  }
}
...
async function atribuujReferral(
  ...
  try {
    ...
    //
    // ⭐ KOUČINK JE VÝJIMKA (rozhodnutí Martina 3. 9. 2026 ráno): provize se platí JEN
    // Z PRVNÍ PLATBY klienta, 10 % jednorázově. Prodloužení ani další nákup koučinku
    // provizi nedává, takže se kontrola dělá i u affiliate S `order_id`.
    // Bez téhle výjimky by partner u Diamondu na měsíc bral 10 % každý měsíc donekonečna.
    // ⚠️ Rozhodovací pravidlo je v `_shared/provize.ts` (`duvodPreskoceniProvize`), aby
    ...
    // vrací s kódem partnera, taky není nový klient. Rozhoduje stav nároku PŘED tímhle
    ...
    if (preskocit) {
      if (jenPrvniPlatba) {
        console.log(
          "provize přeskočena: opakovaný nákup koučinku ("
            + (maRadekProdukt ? "už má řádek v referrals" : "dřívější platba přes Stripe") + ")",
        );
      }
      return preskocit;
    }
...
async function posliUvitani(
  ...
  // ⛔⛔ [17. 9. 2026] CHYBA DOTAZU NENÍ ODPOVĚĎ DOTAZU (CLAUDE.md, pravidlo 13).
  //    Do dneška se `error` zahazoval, takže jeden 504 brány Supabase (ta shazuje
  //    ~1,35 % požadavků, viz `tvujcoach-supabase-504-opakovani`) vypadal úplně
  //    stejně jako „šablona neexistuje": platící člen NEDOSTAL doručovací mail
  //    a alert tvrdil, že chybí text, který ve skutečnosti je.
  //    ⇒ Tři stavy, ne dva: NAŠLA SE / NENÍ / NEVÍM.
  //    Při „nevím" se POKRAČUJE (chyba znamená nevím, ne ne) a Martin dostane alert.
  //    ⚠️ Cena: kdyby šablona opravdu chyběla a zároveň selhalo čtení, zůstane lead
  //       na té trati. To je vzácný souběh a proti tichému nedoručení platícímu
  //       zákazníkovi je to menší škoda.
  const { data: sablona, error: chybaSablony } = await admin
    ...
    .maybeSingle();
  if (chybaSablony) {
    await alertAdmin("Stripe: nešlo ověřit šablonu, mail posílám naslepo", {
      email,
      track: track,
      chyba: String(chybaSablony.message ?? chybaSablony).slice(0, 200),
      co_delat: "Přístup UDĚLEN a mail se odeslat POKUSIL. Ověř v `email_events`, "
        + "jestli opravdu odešel; kdyby ne, pošli ho ručně z adminu.",
    });
  } else if (!sablona) {
...
//
/**
 * Zaplatil ten člověk koučink přes Stripe UŽ DŘÍV, před tímhle nákupem?
 *
 * ⭐ Rozhoduje o provizi: ta se od 3. 9. 2026 platí jen z PRVNÍ platby klienta.
 *
 * ⛔ RUČNÍ GRANT Z ADMINU SE ZA PLATBU NEPOČÍTÁ. Všech 17 dnešních koučinkových nároků
 *    založil admin ručně a žádný z nich nešel přes web; kdyby se braly jako „už platil",
 *    partner by u prvního opravdového prodeje takovému člověku nedostal nic.
 *    Rozlišuje se dvěma znaky, které ruční grant nemá:
 *      1) `source` začíná na `stripe` (KATALOG zapisuje `stripe-koucink`; stejné pravidlo
 *         používá i RPC `koucink_kapacita` v `koucink-stripe.sql`),
 *      2) vyplněné `stripe_payment_intent` z JINÉ platby, než je ta právě probíhající.
 *    Druhý znak je pojistka pro případ, že by se `source` časem přejmenoval.
 */
function koucinkUzPlatilPresStripe(
  // deno-lint-ignore no-explicit-any
  narok: any,
  pi: string | null,
): boolean {
  if (!narok) return false;
  if (String(narok.source ?? "").startsWith("stripe")) return true;
  const drivejsiPi = narok.stripe_payment_intent ? String(narok.stripe_payment_intent) : "";
  return drivejsiPi !== "" && drivejsiPi !== (pi ?? "");
}
...
async function zpracujKoucink(email: string, obj: any, def: JednorazovyProdukt): Promise<Response> {
  ...
  const referral = await atribuujReferral(email, "coaching", cref.kod || null, pi, obj, uzPlatilDriv);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (typ === "charge.refunded" || typ === "charge.dispute.created") {
      ...
      const idsPlateb = idPlatebProRefund(obj);
      const provizeZrusena = idsPlateb.length
        ? await zneplatniProvizeRefundem(
          (q) => q.in("order_id", idsPlateb),
          "order_id " + idsPlateb.join(", "),
          jeSpor,
        )
        : "bez-identifikatoru";
      ...
      if (!ent) {
        ...
        // ⭐ DRUHÁ CESTA: PROVIZE ZA APPKU (doplněno po revizi 3. 9. 2026).
        // ⛔ Párování podle `order_id` výš u appky NIKDY nesedne: most
        //    `app-purchase-bridge` skládá `order_id` z `body.order_id ||
        //    body.payment_intent || event_id` (`core.ts:249`), ale appčí
        //    `stripe-webhook` posílá v těle jen `event_id` (ověřeno 3. 9. 2026 na
        //    `p41-nad-p28`, `supabase/functions/stripe-webhook/index.ts:682` a `:814`).
        //    V `referrals.order_id` tedy stojí `evt_…`, které v refundované události
        //    není. Bez téhle větve by refund appky provizi nezrušil a auto-confirm
        //    by ji po 14 dnech potvrdil.
        // ⚠️ Most se při refundu NEVOLÁ VŮBEC (jediná dvě volání `posliDoMostu` jsou
        //    první platba a obnova), takže druhá strana o vrácení peněz sama neřekne.
        //    Academy tu událost ale VIDÍ: obě appky jedou na jednom Stripe účtu.
        // ⛔ ÚMYSLNĚ AŽ TADY, uvnitř větve „nespárovalo se na žádný nárok Academy".
        //    Refund Academy, koučinku nebo videokurzu se sem nedostane, protože ten
        //    se spároval výš. K tomu filtr `product = 'appka'` a okno
        //    `DNI_OKNO_PROVIZE_APPKY` dní zpátky, ať se nesahá na provizi z první
        //    platby před rokem, kterou partner poctivě dostal.
        // ⚠️ Je to heuristika, ne jistota: `referrals` si ID předplatného neukládá,
        //    takže přesnější klíč v datech neexistuje. Alert řekne, podle čeho se
        //    rušilo, aby to šlo v případě sporu dohledat.
        const oknoAppky = oknoProvizeAppkyIso();
        const provizeAppky = emailZPlatby
          ? await zneplatniProvizeRefundem(
            (q) => q.eq("buyer_email", emailZPlatby).eq("product", "appka")
              .gte("created_at", oknoAppky),
            "appka, e-mail " + emailZPlatby + ", " + DNI_OKNO_PROVIZE_APPKY + " dní zpět",
            jeSpor,
          )
          : "bez-emailu";
        ...
        if (bonusy.length) {
          ...
          return json({
            ...
            provize_appky_zrusena: provizeAppky,
          });
        }
