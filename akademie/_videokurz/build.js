...
<body class="ba">
  <div class="top"><div class="in">
    ...
    <div class="topr"><a class="back" href="/akademie/moje/">← Zpět na Academy</a></div>
  </div></div>
  ...
  <div class="wrap">
    ...
    ${v.free
      ? `<div class="video" id="vidbox"><iframe id="ytframe" src="https://www.youtube-nocookie.com/embed/${v.id}?rel=0&modestbranding=1&enablejsapi=1" title="${esc(v.disp)}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>`
  ...
  <script>
    ...
    var player=null, chain=null, prog={};

    // --- YouTube IFrame API (kvůli detekci konce videa -> autoplay další lekce) ---
    function ytApi(cb){
      if (window.YT && window.YT.Player) return cb();
      window._ytcbs = window._ytcbs || []; window._ytcbs.push(cb);
      if (!window._ytreq){
        window._ytreq = 1;
        window.onYouTubeIframeAPIReady = function(){ (window._ytcbs||[]).forEach(function(f){ try{ f(); }catch(e){} }); };
    ...
    function onState(e){
      ...
      markDone(true);
      var chk=document.getElementById('autoNext');
      if (chk && chk.checked) nextLesson();
    }
    function loadChain(cb){
      if (chain) return cb(chain);
      fetch('/akademie/videokurz/chain.json').then(function(r){ return r.json(); })
        .then(function(j){ chain=j; cb(j); }).catch(function(){});
    }
    function nextLesson(){
    ...
    function swapTo(nx, id, idx){
      ...
      if (nav){
        var p=chain[idx-1], n=chain[idx+1];
        nav.innerHTML=(p?'<a class="btn ghost" href="/akademie/videokurz/'+p.s+'/">← Předchozí</a>':'<span></span>')
          +(n?'<a class="btn ghost" href="/akademie/videokurz/'+n.s+'/">Další →</a>':'<span></span>');
      }
      player.loadVideoById(id);
      try{ window.scrollTo({top:0, behavior:'smooth'}); }catch(e){}
    }
    function overlayBox(html){
      var box=document.getElementById('vidbox'); if(!box) return;
    ...
    function markDone(d){
      ...
      state.done = d; paint(); prog[LID] = d;
      if (GUEST){ lsSet(d); } else if (window.BA){ window.BA.setDone(PRODUCT, LID, d); } else { lsSet(d); }
    }
    // volba autoplay si pamatuje zařízení (výchozí: zapnuto)
    (function(){
      var AK='ba_autoplay_v1', chk=document.getElementById('autoNext');
      try{ chk.checked = localStorage.getItem(AK) !== '0'; }catch(e){}
      chk.addEventListener('change', function(){ try{ localStorage.setItem(AK, chk.checked ? '1' : '0'); }catch(e){} });
    })();
    function load(){
      if (window.BA){
          ...
          if (window.BA.mode==='live'){
            if (FREE) {
              ...
              window.BA.getUser().then(function(u){
                ...
                window.BA.getProgress().then(function(d){ prog=d; state.done=!!d[LID]; paint(); });
              });
            }
            else { window.BA.requireAccess(PRODUCT).then(function(ok){ if(!ok) return; mountVideo(); window.BA.getProgress().then(function(d){ prog=d; state.done=!!d[LID]; paint(); }); }); }
          }
          else { if (FREE) attachFree(); window.BA.getProgress().then(function(d){ prog=d; state.done=!!d[LID]; paint(); }); }
        });
      } else { if (FREE) attachFree(); state.done=lsDone(); paint(); }
    }
    ...
    <div id="vkNext" style="margin-top:2.4rem;">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:14px;">
        <a href="/koucing/" style="text-decoration:none;display:block;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:14px;padding:18px 20px;color:inherit;">
          ...
          <p style="margin:0;font-size:.88rem;color:#B9B3C4;line-height:1.5;">Kurz tě naučil JAK, koučink ti to nastaví na míru a povedu tě týden po týdnu. Vlastní klientská sekce a appka Tvůj Coach v ceně po celou dobu koučinku: generátor jídelníčku i tréninku, 128 cviků a k tomu AI kouč.</p>
        </a>
        <a id="vkNextAca" href="/akademie/" style="text-decoration:none;display:block;background:rgba(235,177,44,.07);border:1px solid rgba(235,177,44,.4);border-radius:14px;padding:18px 20px;color:inherit;">
          ...
          <p style="margin:0;font-size:.88rem;color:#B9B3C4;line-height:1.5;">Barna Academy: 256 lekcí, generátory jídelníčků a tréninků, rok appky Tvůj Coach ve VIP verzi v ceně. Jako majitel videokurzu máš zvýhodněnou cenu: doživotní přístup tě vyjde na <b style="color:#F6CD63;">7 410 Kč</b>.</p>
        </a>
      </div>
    </div>
