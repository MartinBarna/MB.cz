...
if (import.meta.main) {
  ...
  for (const c of cesty) {
    ...
    for (const seg of ['zeny', 'muzi', 'other']) {
      // course_price: drip-send ho plni z konstanty COURSE_PRICE (index.ts), tady jen at token nesvíti jako nerozreseny
      const v = { jmeno: 'Jana', unsubscribe_url: 'https://example.invalid/u', email: 'nikdo@example.invalid', course_price: '1490' };
